export enum Query {
  ...
  'SELECT ship.*, array_agg(role.name) as roles FROM ship INNER JOIN ship_role ON ship.id = ship_role.ship_id INNER JOIN role ON ship_role.role_id = role.id %F GROUP BY ship.id ORDER BY ship.name %P',
